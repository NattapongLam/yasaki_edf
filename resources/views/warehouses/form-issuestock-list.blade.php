        ...
        <table id="tb_job" class="table table-bordered dt-responsive nowrap w-100 text-center">
            ...
            <tbody>
                @foreach ($hd as $item)
                    <tr>
                        <td>
                           @if ($item->wh_issuestock_statuses_id == 1)
                                <span class="bg-warning bg-soft">
                                    {{$item->wh_issuestock_statuses_name}}
                                </span>
                            @elseif($item->wh_issuestock_statuses_id == 2)
                                <span class="bg-danger bg-soft">
                                    {{$item->wh_issuestock_statuses_name}}
                                </span>
                            @elseif($item->wh_issuestock_statuses_id == 3 )
                                <span class="bg-success bg-soft">
                                    {{$item->wh_issuestock_statuses_name}}
                                </span>
                            @endif
                        </td>   
                        <td>{{$item->wh_issuestock_hds_date}}</td>
                        ...
                        <td>
                            ...
                            @else
                                @if ($item->wh_issuestock_statuses_id == 1)
                                    <a href="{{route('issuestocks.show',$item->wh_issuestock_hds_id)}}" class="btn btn-sm btn-primary" >
                        ...
                        <td>
                            @if ($item->wh_issuestock_statuses_id == 1)
                                <a href="{{route('issuestocks.edit',$item->wh_issuestock_hds_id)}}" class="btn btn-sm btn-warning" >
                                    <i class="fas fa-edit"></i>
                                </a> 
                            @endif
                        </td>  
                        <td>
                            @if ($item->wh_issuestock_statuses_id == 1)
                                <a href="javascript:void(0)" class="btn btn-danger btn-sm" onclick="confirmDel('{{ $item->wh_issuestock_hds_id }}')"><i class="fas fa-trash"></i></a>
                            @endif
                        </td>  
                    </tr>
                @endforeach
