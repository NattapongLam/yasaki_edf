...
<div class="container-fluid">
    ...
    <table class="table table-bordered">
        <tr>
            ...
            <td>{{ $hd->log_ms_formule_name }}</td>

            <th width="150">วันที่</th>
            <td>{{ \Carbon\Carbon::parse($hd->log_update_at)->format('d/m/Y H:i') }}</td>
        </tr>
        <tr>
            <th>ประเภท</th>
            <td>{{ $hd->log_chemistry_hd_type }}</td>

            <th>เลขที่สูตร</th>
            <td>{{ $hd->log_chemistry_hd_name }} V.{{$hd->log_version}}</td>
        </tr>
        <tr>
            <th>Mixing (kg/Batch)</th>
            <td>{{ number_format($hd->log_chemistry_hd_mix,2) }}</td>

            <th>Total (W)</th>
            <td>{{ number_format($hd->log_chemistry_hd_qty,2) }}</td>
        </tr>
        <tr>
            <th>หมายเหตุ</th>
            <td colspan="3">{{ $hd->log_chemistry_hd_note }}</td>
        </tr>
    </table>
    ...
    <table class="table table-bordered">
        ...
        <tbody id="tableBody">
            @foreach($dt as $r)
            <tr>
                <td class="text-center">{{ $r->log_no }}</td>
                <td>{{ $r->log_code }}</td>
                <td>{{ $r->log_material }}</td>
                <td>{{ $r->log_grade }}</td>
                <td class="text-end"><input type="hidden" class="density" value="{{ $r->log_density }}">{{ number_format($r->log_density,2) }}</td>
                <td class="text-end"><input type="hidden" class="adjust" value="{{ $r->log_adjust }}">{{ number_format($r->log_adjust,2) }}</td>
                <td class="text-end"><input type="hidden" class="weght" value="{{ $r->log_weght }}">{{ number_format($r->log_weght,2) }}</td>
                <td class="text-end"><input type="hidden" class="weghtper" value="{{ $r->log_weghtper }}">{{ number_format($r->log_weghtper,2) }}</td>
                <td class="text-end"><input type="hidden" class="weghttotal" value="{{ $r->log_weghttotal }}">{{ number_format($r->log_weghttotal,2) }}</td>
