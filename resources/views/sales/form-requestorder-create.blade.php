            ...
            <div class="col-3">
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" 
                            ...
                            value="{{ old('ar_requestorder_hds_date', now()->format('Y-m-d')) }}" 
                            required>
                </div>
            </div>
            <div class="col-3">
...
<script>
function loadDocNo() {
    let docDate = $("#ar_requestorder_hds_date").val();

    if (docDate) {
        $.ajax({
            url: "{{ route('requestorder.runno') }}",
            type: "GET",
            data: { date: docDate },
            success: function (res) {
                $("#ar_requestorder_hds_docuno").val(res.docno);
            }
        });
    }
}
...
// เปลี่ยนวันที่
$("#ar_requestorder_hds_date").on('change', function () {
    loadDocNo();
});
