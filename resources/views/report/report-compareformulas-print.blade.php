...
.chart-box {
    height: 155px;
    width: 80%; /* หรือปรับขนาดความกว้างตามต้องการ เช่น 85% หรือ 100% */
    max-width: 800px; /* กำหนดขนาดความกว้างสูงสุดไม่ให้กว้างเกินไป */
    margin: 6px auto; /* ใช้ auto ช่วยดันซ้าย-ขวาให้อยู่กึ่งกลาง */
    border: 1px solid var(--hairline);
    border-radius: 3px;
...
<div class="header">
    <div class="header-top">
        ...
        <div class="accred-badge">
            ...
            ISO/IEC 17025<br>
            YSK5-FM-LAB-15 : Rev.00 : 01/08/2569
        </div>
    </div>
    <div class="report-title">Coefficient of Friction Test Report</div>
    <div class="report-subtitle">Coefficient of Friction Test Report</div>
...
<div class="section-title" style="margin-top:0;">Sample Details</div>
...
<table class="spec-photo" style="margin-top:6px;">
    <tr>
        ...
        <td class="text-center"><img src="{{asset($rechd->receive_test_lists_file1)}}" class="img-thumbnail" width="38%"></td>
        <td class="text-center"><img src="{{asset($rechd->receive_test_lists_file2)}}" class="img-thumbnail" width="38%"></td>    
    </tr>
    <tr>
...
<table class="spec-photo" style="margin-top:6px;">
    <tr>
        ...
        <td class="text-center"><img src="{{asset($rechd->result_test_lists_file1)}}" class="img-thumbnail" width="38%"></td>
        <td class="text-center"><img src="{{asset($rechd->result_test_lists_file2)}}" class="img-thumbnail" width="38%"></td>
    </tr>
    <tr>
        <th colspan="2">Harmful crack, Swelling,<br>Scratch, Unevenness, Warp, Twist</th>
...
<div class="section-title" style="margin-top:0;">Detailed Test Results</div>
...
<!-- กราฟ Friction -->
<div class="no-break" style="margin-top:6px; text-align: center;">
    <div class="chart-box" style="margin: 0 auto;">
        <canvas id="frictionChart"></canvas>
        <div class="chart-title">Note: Coefficient of Friction (COF) unit is represented as µ</div>
    </div>   
</div>
...
<div class="no-break" style="margin-top:6px; text-align: center;">
    <div class="chart-box" style="margin: 0 auto;">
        ...
        <div class="chart-title">Note: Wear Rate unit is represented as ( 10^-7 cm^3/N.m )</div>
    </div>
</div>
...
labels:temps,
datasets:[
{data:safeUpper,borderWidth:0,pointRadius:0,backgroundColor:'rgba(61,109,153,0.10)',fill:'+1'},
{data:safeLower,borderWidth:0,pointRadius:0,fill:false},
{data:jisMax,borderColor:'#b3261e',borderDash:[6,6],borderWidth:1,pointRadius:0},
{data:jisMin,borderColor:'#b3261e',borderDash:[6,6],borderWidth:1,pointRadius:0},
{data:targetUpper,borderColor:'#3d6d99',borderDash:[2,2],borderWidth:1,pointRadius:0},
{data:targetLower,borderColor:'#3d6d99',borderDash:[2,2],borderWidth:1,pointRadius:0},
{label:'Friction Avg',data:frictionPoints,borderColor:'#122844',backgroundColor:'#122844',borderWidth:2,pointRadius:3,tension:0.2}
]
},
options:{
responsive:true,
...
y:{title:{display:true,text:'Friction (μ)'},min:0,max:0.8,ticks:{stepSize:0.2}}
}
}
});
...
data:{
labels:temps,
datasets:[{label:'Wear Rate Avg',data:wearRatePoints,borderColor:'#122844',backgroundColor:'#122844',borderWidth:2,pointRadius:3,tension:0.2}]
},
options:{
responsive:true,
maintainAspectRatio:false,
animation:false,
plugins:{legend:{display:false}},
scales:{y:{title:{display:true,text:'Wear Rate (10⁻⁷ cm³/N·m)'},min:0,max:1.6,ticks:{stepSize:0.2}}}
}
});
...
function buildPieData(){
    ...
    const fallbackPalette = ['#1c3a5e','#3d6d99','#6f9bc4','#a9c6e0','#b3261e','#e0a458','#4a7c59','#8e7cc3'];
    const groups = {};
    let fallbackIdx = 0;
